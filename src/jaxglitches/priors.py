...
_BETA_MIN   = 0.01    # minimum decay timescale (s)
_BETA_MAX   = 1000.0  # maximum decay timescale (s)
...
def prior_inverse_cdf(u, t_obs: float = T_OBS_s):
    ...
    t_obs : observation window length (s); upper bound for tau.

    Returns
    -------
    params : array of shape (..., 3) = [tau, Deltav, beta].
    """
    u_tau, u_Dv, u_beta = u[..., 0], u[..., 1], u[..., 2]

    tau   = u_tau * t_obs
    Deltav = jnp.exp(jnp.log(_DELTAV_MIN) + u_Dv * jnp.log(_DELTAV_MAX / _DELTAV_MIN))
    beta  = jnp.exp(jnp.log(_BETA_MIN)   + u_beta * jnp.log(_BETA_MAX  / _BETA_MIN))

    return jnp.stack([tau, Deltav, beta], axis=-1)


def log_prior(params, t_obs: float = T_OBS_s):
    """
    Log prior density p(tau, Deltav, beta).
    ...
    """
    tau, Deltav, beta = params[..., 0], params[..., 1], params[..., 2]

    # uniform in tau
    log_p_tau = -jnp.log(t_obs)

    # log-uniform in Deltav and beta
    log_p_Dv   = -jnp.log(Deltav)  - jnp.log(jnp.log(_DELTAV_MAX / _DELTAV_MIN))
    log_p_beta = -jnp.log(beta)    - jnp.log(jnp.log(_BETA_MAX   / _BETA_MIN))

    in_bounds = (
        (tau   >= 0.0)         & (tau   <= t_obs)
        & (Deltav >= _DELTAV_MIN) & (Deltav <= _DELTAV_MAX)
        & (beta  >= _BETA_MIN)  & (beta  <= _BETA_MAX)
    )

    lp = log_p_tau + log_p_Dv + log_p_beta
...
def prior_bounds(t_obs: float = T_OBS_s):
    ...
    lower = jnp.array([0.0,        _DELTAV_MIN, _BETA_MIN])
    upper = jnp.array([t_obs,      _DELTAV_MAX, _BETA_MAX])
    return lower, upper
